...
function openMenu() {
    $(".menu").click(function() {
        if(hasClick == false) {
            ...
            $("#contact-re").css("color", "#E4380A")
            $("#contact-re").append("<span class = 'pointer-location'><span class = 'pointer-location-slash'>&nbsp;/</span>you are here</span>")
            $("#contact-re").css("pointer-events", "none")
...
function contactHover() {
   ...
   $(".contact-header a").hover(
    function() {
     ...
     var classB = "." + $(this).attr("class")
     console.log(classB)
     distort1 = baffle(classB)
     distort1.set({
         characters: 'XOXO1313',
         speed: 100
     })
     distort1.start()
     distort1.reveal(1000)
    },
    function() {
        console.log("stop")
        distort1.stop()
        $(".contact-header span").text($(".contact-header span").attr("class"))
    }
)
}
...
.from(".contact-text span", 2, {opacity:0, stagger:0.1, y: "30%", ease: Power4.easeOut}, 0)
.from(".contact-href", 3,{opacity:0, x: "25%", stagger: 0.2, ease: Power4.easeOut},0)
.from(".contact-header span",3,{opacity:0, right: "-10%", stagger: 0.2, ease: Power4.easeOut},0 )
